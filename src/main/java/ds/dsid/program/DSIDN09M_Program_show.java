...
public class DSIDN09M_Program_show extends COMM_Master {
	...
	private Textbox txt_UNIQUEID,txt_PO_NO,txt_PO_SEQ,txt_EL_NO,txt_EL_CNAME,
					txt_PO_NOSQL,txt_PO_SEQSQL,txt_PO_NOa,txt_PC_QTY,txt_Pqq;
	@Wire
	private Input txtb;
    ...
    public String aaa="sdsddssd";
    public DSIDN08 d08;
    public DSIDN06 d06;
    public String PLACE;
    public String MODEL_NA;
    
    
    
	
public DSIDN06 getD06() {
		return d06;
	}

	public void setD06(DSIDN06 d06) {
		this.d06 = d06;
	}

public DSIDN08 getD08() {
		return d08;
	}

	public void setD08(DSIDN08 d08) {
		this.d08 = d08;
	}
	...
	public ComponentInfo doBeforeCompose(Page page, Component parent, ComponentInfo compInfo) {
		...
		d06 =  getNewMessage02();//getNewMessage();//new DSIDN06();//
		aaa="doBeforeCompose";
		d08 = new DSIDN08();
	...
	protected Class getMasterClass() {
		return DSIDN09M_Program_show.class;
	}
...
//		}
		 ...
		 * @return 返回序列
		 */
		private DSIDN06 getNewMessage(){
			DSIDN06 dt06 = new DSIDN06();
			CRUDService CRUDServicetemp = (CRUDService) SpringUtil.getBean("CRUDService");
			String sql ="select A1,A2,A3,"
					+" B1,B2,B3,B4,B5,B6,B7,B8,B9,B10,B11,B12,"
					+" C1,C2,C3,D1,D2,D3,E1,E2,E3,F1,F2,F3,G1,G2,G3,G4,G5,"
					+" H1,H2,H3,H4,H5 "
					+" from DSIDN06  where  areanum =(select max(areanum) from DSIDN06)";
			Query  tempquery = CRUDServicetemp.createSQL(sql);
			List	tempList = tempquery.getResultList();
			for(int i =0;i<tempList.size();i++){
				Object [] ot = (Object[]) tempList.get(i);
				for(int x =0;x<ot.length;x++){
					String obj = String.valueOf(ot[x]);
					initValue(x,dt06,obj);
				}
			}
		...
		private void initValue(int index,DSIDN06 dt06,String value){
			if(index==0)dt06.setA1(value);
			if(index==1)dt06.setA2(value);
			if(index==2)dt06.setA3(value);
			
			if(index==3)dt06.setB1(value);
			if(index==4)dt06.setB2(value);
			if(index==5)dt06.setB3(value);
			if(index==6)dt06.setB4(value);
			if(index==7)dt06.setB5(value);
			if(index==8)dt06.setB6(value);
			if(index==9)dt06.setB7(value);
			if(index==10)dt06.setB8(value);
			if(index==11)dt06.setB9(value);
			if(index==12)dt06.setB10(value);
			if(index==13)dt06.setB11(value);
			if(index==14)dt06.setB12(value);
			
			if(index==15)dt06.setC1(value);
			if(index==16)dt06.setC2(value);
			if(index==17)dt06.setC3(value);
			
			if(index==18)dt06.setD1(value);
			if(index==19)dt06.setD2(value);
			if(index==20)dt06.setD3(value);
			
			if(index==21)dt06.setE1(value);
			if(index==22)dt06.setE2(value);
			if(index==23)dt06.setE3(value);

			if(index==24)dt06.setF1(value);
			if(index==25)dt06.setF2(value);
			if(index==26)dt06.setF3(value);
			
			if(index==27)dt06.setG1(value);
			if(index==28)dt06.setG2(value);
			if(index==29)dt06.setG3(value);
			if(index==30)dt06.setG4(value);
			if(index==31)dt06.setG5(value);
			
			if(index==32)dt06.setH1(value);
			if(index==33)dt06.setH2(value);
