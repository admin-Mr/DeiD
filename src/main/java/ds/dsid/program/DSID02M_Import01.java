...
public class DSID02M_Import01 extends QueryBase {
	@WireVariable
	...
	@Wire
	private Listbox queryListbox,abclist;
	@Wire
	...
	@Wire
	private Textbox  txtSEASON, txtSTAGE, txtSPEC_NO;
	@Wire
	...
	public void doAfterCompose(Component window) throws Exception {
		...
		ww.setWidth("400px"); //400
		ww.setHeight("800px");// 200
		...
		for(int i =0;i<10;i++){
			ZKTESTVO abc = new  ZKTESTVO();
			abc.setZK_ID(i+"abc");
			list.add(abc);
		}
		
		ListModelList listmodel = new ListModelList(list, true); 
		abclist.setModel(listmodel);
...
//		}
	...
	public void onClickbtnComfig(Event event){
		/*//String str = 	((Textbox)getRootWindow().getFellow("txtMODEL_NAME")).getValue();
	//	System.out.println("====================str=-----------"+str);
		 ...
		 * EL_NO       VARCHAR2(50) not null,
  EL_MODEL    VARCHAR2(150) not null,
  ...
  UNIQUEID    NUMBER not null
		...
		for(int i=0;i<inlist.size();i++){
			...
			String sql ="insert into DSID02(UNIQUEID,PART_NAME,EL_NO,COLOR,EL_MODEL,CREATE_DATE,UP_DATE,CREATE_USER,STATUS) values(seq_dsid02.Nextval,?,?,?,?,sysdate,sysdate,'"+_userInfo.getAccount()+"',100)";
			try {
    ...
    public List<List<Object>> importExcel(Media media){
    	List<Object[]> list = DSID02M_Import01_Excel.importExcel(media.getStreamData(), 0,4);
