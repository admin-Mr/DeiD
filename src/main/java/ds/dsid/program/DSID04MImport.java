...
public class DSID04MImport extends OpenWinCRUD {
	...
	public void importFromExcel(InputStream input) throws Exception {
		...
		try {
			...
			for (int i = 1; i < wb.getNumberOfSheets(); i++) {
				String WbName = wb.getSheetName(i).toUpperCase();
				if ("VAMP".equals(WbName)) {
					ImportSheet1(i, wb, conn);// VAMP 鞋面
				} else if ("LABEL".equals(WbName)) {
					ImportSheet2(i, wb, conn);// label 標籤
				} else if ("LACE".equals(WbName)) {
					ImportSheet3(i, wb, conn);// lace 鞋帶
				} else if ("HEEL CLIP".equals(WbName)) {
					ImportSheet4(i, wb, conn);// heel clip 腳跟夾
				}
			}
	...
	private void ImportSheet4(int sheet_i, HSSFWorkbook wb, Connection conn) throws SQLException {
		// TODO Auto-generated method stub
		System.out.println(">>>" + sheet_i + ">>>>>>>>heel clip>>>>");
		int Start_Row = 0;
		DateFormat Format = new SimpleDateFormat("yyyy/MM/dd");
		HSSFFormulaEvaluator Formul = new HSSFFormulaEvaluator(wb);
		HSSFSheet sheet = wb.getSheetAt(sheet_i);
		...
		try {
				...
				if (Str1.contains("Length") || Str1.contains("長度")) {
					...
					for (int j = 2; j < 32; j++) {
						...
						if (!"".equals(EL_NO) && EL_NO != null) {
							...
							String sql2 = "INSERT INTO DSID04_5 (MODEL_NA,EL_SEQ,EL_NO,COLOR,EL_NA,COLOR_PRE,MIN_DAY,INI_BUY,UP_USER,UP_DATE) VALUES ('"
									+ MODEL_NA + "','" + EL_SEQ + "','" + EL_NO + "','" + COLOR + "','" + EL_NA + "','"
									+ COLOR_PRE + "','" + MIN_DAY + "','" + INI_BUY + "','" + _userInfo.getAccount()
									+ "',TO_DATE('" + Format.format(new Date()) + "','YYYY/MM/DD'))";
							...
							try {
								...
								Errmessage = Labels.getLabel("DSID.MSG0055") + "5" + Labels.getLabel("DSID.MSG0058")
										+ EL_NO + Labels.getLabel("DSID.MSG0054") + "！" + e;
								conn.rollback();
								e.printStackTrace();
			...
			if (Errmessage.length() <= 0) {
				conn.commit();
				Okmessage += "HEEL CLIP " + Labels.getLabel("DSID.MSG0056") + SEQ + Labels.getLabel("DSID.MSG0057")
						+ "！\n";
			} else {
				conn.rollback();
			}
	...
	private void ImportSheet3(int sheet_i, HSSFWorkbook wb, Connection conn) throws SQLException {
		// TODO Auto-generated method stub
		System.out.println(">>>讀取工作表" + sheet_i + ">>>>>>>鞋帶>>>>>");
		int Start_Row = 0;
		DateFormat Format = new SimpleDateFormat("yyyy/MM/dd");
		HSSFFormulaEvaluator Formul = new HSSFFormulaEvaluator(wb);
		HSSFSheet sheet = wb.getSheetAt(sheet_i);
		...
		try {
				...
				if (Str1.contains("Length") || Str1.contains("長度")) {
					...
					for (int j = 2; j < 22; j++) {
						...
						if (!"".equals(EL_NO) && EL_NO != null) {
							...
							String sql2 = "INSERT INTO DSID04_4 (MODEL_NA,EL_SEQ,EL_NO,COLOR,EL_NA,COLOR_PRE,MIN_DAY,INI_BUY,UP_USER,UP_DATE) VALUES ('"
									+ MODEL_NA + "','" + EL_SEQ + "','" + EL_NO + "','" + COLOR + "','" + EL_NA + "','"
									+ COLOR_PRE + "','" + MIN_DAY + "','" + INI_BUY + "','" + _userInfo.getAccount()
									+ "',TO_DATE('" + Format.format(new Date()) + "','YYYY/MM/DD'))";
							...
							try {
								...
								Errmessage = "LACE " + Labels.getLabel("DSID.MSG0058") + EL_NO
										+ Labels.getLabel("DSID.MSG0054") + "！" + e;
								conn.rollback();
								e.printStackTrace();
			...
			if (Errmessage.length() <= 0) {
				conn.commit();
				Okmessage += "LACE " + Labels.getLabel("DSID.MSG0056") + SEQ + Labels.getLabel("DSID.MSG0057") + "！\n";
			} else {
				conn.rollback();
			}
	...
	private void ImportSheet2(int sheet_i, HSSFWorkbook wb, Connection conn) throws SQLException {
		// TODO Auto-generated method stub
		System.out.println(">>>讀取工作表" + sheet_i + ">>>>>>>>布標>>>>>>");
		int Start_Row = 0;
		String ET_US = "";
		DateFormat Format = new SimpleDateFormat("yyyy/MM/dd");
		HSSFFormulaEvaluator Formul = new HSSFFormulaEvaluator(wb);
		...
		if (sheet.getPhysicalNumberOfRows() != 0) {
			...
			if (!"".equals(startStr)) {
				...
				try {
					...
					for (int x = 0; x < 10; x++) {
						...
						if (Str2.contains("estimated usage")) {
							ET_US = Str2.replace("預計用量 (estimated usage ", "").replace("預計用量 (estimated usage", "")
									.replace(")", "");
						}
					}
					...
					for (int i = Start_Row; i < sheet.getPhysicalNumberOfRows(); i++) {
						...
						String sql2 = "INSERT INTO DSID04_3 (MODEL_NA,EL_SEQ,COLOR,EL_NO,EL_NA,YIELD,COLOR_PRE,INI_BUY,ET_US,UP_USER,UP_DATE) VALUES ('"
								+ MODEL_NA + "','" + EL_SEQ + "','" + COLOR + "','" + EL_NO + "','" + EL_NA + "','"
								+ YIELD + "','" + COLOR_PRE + "','" + INI_BUY + "','" + ET_US + "','"
								+ _userInfo.getAccount() + "',TO_DATE('" + Format.format(new Date())
								+ "','YYYY/MM/DD'))";
						...
						try {
							...
							Errmessage = Labels.getLabel("DSID.MSG0055") + "3" + Labels.getLabel("DSID.MSG0058") + EL_NO
									+ Labels.getLabel("DSID.MSG0054") + "！" + e;
							conn.rollback();
							e.printStackTrace();
					...
					if (Errmessage.length() <= 0) {
						conn.commit();
						Okmessage += "LABEL " + Labels.getLabel("DSID.MSG0056") + SEQ + Labels.getLabel("DSID.MSG0057")
								+ "\n";
					} else {
						conn.rollback();
					}
				...
				Okmessage += "LABEL " + Labels.getLabel("DSID.MSG0059") + "！\n";
			}
		} else {
			System.out.println("工作表3無資料.");
			Okmessage += "LABEL " + Labels.getLabel("DSID.MSG0059") + "！\n";
	...
	private void ImportSheet1(int sheet_i, HSSFWorkbook wb, Connection conn) throws SQLException {
		// TODO Auto-generated method stub
		System.out.println(">>>讀取工作表" + sheet_i + ">>>>>>>前面片>>>>>>>");
		int Start_Row = 0;
		String ET_US = "";
		DateFormat Format = new SimpleDateFormat("yyyy/MM/dd");
		HSSFFormulaEvaluator Formul = new HSSFFormulaEvaluator(wb);
			...
			if (!"".equals(startStr)) {
				...
				try {
					...
					for (int i = Start_Row; i < sheet.getPhysicalNumberOfRows(); i++) {
						...
						String sql2 = "INSERT INTO DSID04_2 (MODEL_NA,EL_SEQ,COLOR,EL_NO,EL_NA,YIELD,COLOR_PRE,DAILY_UOM,INI_BUY,ET_US,UP_USER,UP_DATE) VALUES ('"
								+ MODEL_NA + "','" + EL_SEQ + "','" + COLOR + "','" + EL_NO + "','" + EL_NA + "','"
								+ YIELD + "','" + COLOR_PRE + "','" + DAILY_UOM + "','" + INI_BUY + "','" + ET_US
								+ "','" + _userInfo.getAccount() + "',TO_DATE('" + Format.format(new Date())
								+ "','YYYY/MM/DD'))";
						...
						try {
							...
							Errmessage = Labels.getLabel("DSID.MSG0055") + "2" + Labels.getLabel("DSID.MSG0058") + EL_NO
									+ Labels.getLabel("DSID.MSG0054") + "！" + e;
							conn.rollback();
							e.printStackTrace();
					...
					if (Errmessage.length() <= 0) {
						conn.commit();
						Okmessage += "VAMP " + Labels.getLabel("DSID.MSG0056") + SEQ + Labels.getLabel("DSID.MSG0057")
								+ "！\n";
					} else {
						conn.rollback();
					}
				...
				Okmessage += "VAMP " + Labels.getLabel("DSID.MSG0059") + "！\n";
			}
		} else {
			System.out.println("工作表2無資料.");
			Okmessage += "VAMP " + Labels.getLabel("DSID.MSG0059") + "！\n";
	...
	private void ImportSheet0(HSSFWorkbook wb, Connection conn) throws SQLException {
		...
		try {
			...
			String sql1 = "INSERT INTO DSID04 (MODEL_NA,LA_DATE,DR_DATE,MODEL_UPD,UP_USER,UP_DATE) " + "VALUES ('"
					+ MODEL_NA + "',TO_DATE('" + LA_DATE + "','YYYY/MM/DD'),TO_DATE('" + DR_DATE + "','YYYY/MM/DD'),'"
					+ MODEL_UPD + "','" + _userInfo.getAccount() + "',TO_DATE('" + Format.format(new Date())
					+ "','YYYY/MM/DD'))";
			...
			for (int i = Start_Row; i < sheet.getPhysicalNumberOfRows(); i++) {
				...
				String sql2 = "INSERT INTO DSID04_1 ( MODEL_NA,EL_SEQ,EL_NO,GROUP_NO,MT_USAGE,SU_NA,EL_NA,GR_NA,COLOR,ELLA_DATE,ELDR_DATE,LA_TIME,EL_UNIT,MTLDAY,SU_MOQ,PO_PRICE,YIELD,COLOR_PRE,\n"
						...
						+ MODEL_NA + "','" + EL_SEQ + "','" + EL_NO + "','" + GROUP_NO + "','" + MT_USAGE + "','"
						+ SU_NA + "','" + EL_NA + "','" + GR_NA + "','" + COLOR + "',TO_DATE('" + ELLA_DATE
						+ "','YYYY/MM/DD'),TO_DATE('" + ELDR_DATE + "','YYYY/MM/DD'),TO_DATE('" + LA_TIME
						+ "','YYYY/MM/DD'),'" + EL_UNIT + "','" + MTLDAY + "','" + SU_MOQ + "','" + PO_PRICE + "','"
						+ YIELD + "','" + COLOR_PRE + "',\n" + "'" + DAILY_UOM + "','" + SAFE_DAY + "','" + MIN_UOM
						+ "','" + MIN_DAY + "','" + MAX_UOM + "','" + MAX_DAY + "','" + _userInfo.getAccount()
						+ "',TO_DATE('" + Format.format(new Date()) + "','YYYY/MM/DD'))";
				...
				try {
					...
					Errmessage = Labels.getLabel("DSID.MSG0055") + "1" + Labels.getLabel("DSID.MSG0058") + EL_NO
							+ Labels.getLabel("DSID.MSG0054") + "！" + e;
					conn.rollback();
					e.printStackTrace();
			...
			if (Errmessage.length() <= 0) {
				conn.commit();
				Okmessage += "UPPER " + Labels.getLabel("DSID.MSG0056") + SEQ + Labels.getLabel("DSID.MSG0057") + "\n";
			} else {
				conn.rollback();
			}
	...
	private void DeleteModel_na(String MODEL_NA, Connection conn) throws SQLException {
		...
		String sql6 = "DELETE DSID04_5 WHERE MODEL_NA='" + MODEL_NA + "'";
