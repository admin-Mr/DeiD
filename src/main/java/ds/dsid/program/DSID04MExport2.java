...
public class DSID04MExport2 extends OpenWinCRUD {
	...
	private void filterHeader() throws SQLException {
		...
		if (!"".equals(MODEL_NA)) {
			...
			HSSFSheet sheet1 = null, sheet2 = null, sheet3 = null, sheet4 = null, sheet5 = null;
			...
			try {
				...
				// 儲存位置、名稱
				Filedownload.save(is, "application/xls", MODEL_NA);
				is.close();
				stream.flush();
				stream.close();
...
//				+ mODEL_NA + "'";
		...
		try {
			...
			while (rs2.next()) {
				if (cellNum % 30 == 2) {
					...
					// 表頭設定, 大小表頭
					Header5(sheet5, style9, row, cell, rowNum, ET_US);
					// 統計欄

					row = sheet5.getRow(rowNum);
					cell = row.createCell(1);
					cell.setCellStyle(style2);

					row = sheet5.getRow(rowNum + 1);
					cell = row.createCell(1);
					cell.setCellStyle(style2);

					row = sheet5.getRow(rowNum + 2);
					cell = row.createCell(1);
					cell.setCellStyle(style2);

					// 庫存
					row = sheet5.getRow(rowNum + 3);
					cell = row.createCell(1);
					cell.setCellStyle(style3);

					row = sheet5.getRow(rowNum + 4);
					cell = row.createCell(1);
					cell.setCellStyle(style3);

					row = sheet5.getRow(rowNum + 5);
					cell = row.createCell(1);
					cell.setCellStyle(style7);

					// 在途
					row = sheet5.getRow(rowNum + 6);
					cell = row.createCell(1);
					cell.setCellType(1);
					cell.setCellStyle(style3);
					...
					row = sheet5.getRow(rowNum + 8);
					cell = row.createCell(1);
					cell.setCellType(1);
					cell.setCellStyle(style8);
					cell.setCellFormula("SUM(C" + (rowNum + 9) + ":V" + (rowNum + 9) + ")");

					row = sheet5.getRow(rowNum + 9);
					cell = row.createCell(1);
					cell.setCellStyle(style3);

					row = sheet5.getRow(rowNum + 10);
					cell = row.createCell(1);
					cell.setCellType(1);
					cell.setCellStyle(style3);
					cell.setCellFormula("SUM(C" + (rowNum + 11) + ":V" + (rowNum + 11) + ")");

					row = sheet5.getRow(rowNum + 11);
					cell = row.createCell(1);
					cell.setCellStyle(style3);

					row = sheet5.getRow(rowNum + 12);
					cell = row.createCell(1);
					cell.setCellStyle(style3);

					row = sheet5.getRow(rowNum + 13);
					cell = row.createCell(1);
					cell.setCellStyle(style3);

					row = sheet5.getRow(rowNum + 14);
					cell = row.createCell(1);
					cell.setCellType(1);
					cell.setCellStyle(style3);
					cell.setCellFormula("SUM(C" + (rowNum + 15) + ":V" + (rowNum + 15) + ")");

					if (Integer.valueOf(His.get(1).toString()) > 0) {
				...
				if (Last_color.compareTo(rs2.getString("COLOR")) == 0) {
					// System.err.println(">>>合併 行"+rowNum+" >>> 行 "+ rowNum+"
					// >>>列 "+ c_Num+" >>>列 "+ cellNum);
					sheet5.addMergedRegion(new CellRangeAddress(rowNum, rowNum, c_Num, cellNum));
				} else {
					c_Num = cellNum;
				...
				if (cellNum < 26) {
					...
					cell.setCellFormula("$L$3*A" + ((char) (cellNum + 39)) + (rowNum + 6));
				}
				...
				if (cellNum < 26) {
					...
					cell.setCellFormula("A" + ((char) (cellNum + 39)) + (rowNum + 8) + "*" + ET_US + "");
				}
				...
				cell.setCellStyle(style3);
				if (cellNum < 26) {
					cell.setCellFormula("" + ((char) (cellNum + 65)) + (rowNum + 9) + "-(" + ((char) (cellNum + 65))
							+ (rowNum + 4) + "+" + ((char) (cellNum + 65)) + (rowNum + 7) + ")");
				} else {
					cell.setCellFormula("A" + ((char) (cellNum + 39)) + (rowNum + 9) + "-(A" + ((char) (cellNum + 39))
							+ (rowNum + 4) + "+A" + ((char) (cellNum + 39)) + (rowNum + 7) + ")");
				}
				...
				cell.setCellStyle(style3);
				if (cellNum < 26) {
					cell.setCellFormula("" + ((char) (cellNum + 65)) + (rowNum + 4) + "/(" + ((char) (cellNum + 65))
							+ ((rowNum + 6)) + "*$L$3)");
				} else {
					cell.setCellFormula("A" + ((char) (cellNum + 39)) + (rowNum + 4) + "/(A" + ((char) (cellNum + 39))
							+ ((rowNum + 6)) + "*$L$3)");
				}
				...
				if (cellNum < 26) {
					...
					cell.setCellFormula("A" + ((char) (cellNum + 39)) + (rowNum + 5));
				}
				row = sheet5.getRow(rowNum + 14);
				cell = row.createCell(cellNum);
				cell.setCellType(1);
				...
				if (Integer.valueOf(His.get(1).toString()) > 0) {
					for (int k = 0; k < (Integer.valueOf(His.get(1).toString()) + 1); k++) {
						...
						if (!is_buy) {
							...
							cell.setCellType(1);
							cell.setCellStyle(style3);
							String MMess = GetHisQty(MODEL_NA, rs2.getString("EL_NO"), conn, k);
							cell.setCellValue(MMess);
						}
					}
				}
	...
	private void Header5(HSSFSheet sheet5, HSSFCellStyle style9, HSSFRow row, HSSFCell cell, int rowNum, String ET_US) {
		...
		HSSFSheetConditionalFormatting sheetCF4 = sheet5.getSheetConditionalFormatting();
		...
		row = sheet5.createRow(rowNum + 10);
		row.setHeightInPoints(20);
		cell = row.createCell(0);
		cell.setCellType(1);
		cell.setCellStyle(style9);
		...
		row = sheet5.createRow(rowNum + 11);
		row.setHeightInPoints(20);
		cell = row.createCell(0);
		cell.setCellType(1);
		cell.setCellStyle(style9);
		...
		row = sheet5.createRow(rowNum + 12);
		row.setHeightInPoints(20);
		cell = row.createCell(0);
		cell.setCellType(1);
		cell.setCellStyle(style9);
		...
		row = sheet5.createRow(rowNum + 13);
		row.setHeightInPoints(20);
		cell = row.createCell(0);
		cell.setCellType(1);
		cell.setCellStyle(style9);
		...
		row = sheet5.createRow(rowNum + 14);
		row.setHeightInPoints(20);
		cell = row.createCell(0);
		cell.setCellType(1);
		cell.setCellStyle(style9);
		...
		if (Integer.valueOf(His.get(1).toString()) > 0) {
			for (int k = 0; k < (Integer.valueOf(His.get(1).toString()) + 1); k++) {
				row = sheet5.createRow(rowNum + 15 + k);
		...
		row = sheet5.createRow(rowNum + 15 + (Integer.valueOf(His.get(1).toString()) + 2));
		cell = row.createCell(0);
		cell.setCellType(1);
	}
	//設置表4
	private void SetSheet4(HSSFWorkbook wb, HSSFSheet sheet4, HSSFCellStyle style9, HSSFCellStyle style2,
		...
		for (int i = 2; i < 22; i++) {
			sheet4.setColumnWidth(i, 14 * 256);
		}
		...
		try {
			...
			while (rs1.next()) {
				...
				cell.setCellType(1);
				cell.setCellStyle(style2);
				cell.setCellValue(rs1.getString("EL_NA"));
	...
	private void SetSheet3(HSSFWorkbook wb, HSSFSheet sheet3, HSSFCellStyle style1, HSSFCellStyle style2,
		...
		try {
			...
			while (rs2.next()) {
				...
				if (Integer.valueOf(His.get(1).toString()) > 0) {
					for (int k = 0; k < (Integer.valueOf(His.get(1).toString()) + 1); k++) {
						boolean is_buy = Check_buy(MODEL_NA, "DSID04_3", conn, k);
						if (!is_buy) {
							sheet3.setColumnHidden(11 + k, true); // 隐藏空白列
						} else {
		...
		// //隐藏空白列
		// if (Integer.valueOf(His.get(1).toString()) > 0) {
		// for (int k = 0; k < (Integer.valueOf(His.get(1).toString()) + 1);
		// k++) {
		// boolean is_buy=Check_buy(MODEL_NA, "DSID04_3", conn, k);
		// if(!is_buy){
		// sheet3.setColumnHidden(11 + k, true);
		// }
		// }
		// }
	...
	private void Header3(HSSFSheet sheet3, HSSFCellStyle style1, HSSFRow row, HSSFCell cell) {
		...
		for (int k = 0; k < (Integer.valueOf(His.get(1).toString()) + 1); k++) {
			sheet3.setColumnWidth(11 + k, 11 * 256);
		}
	...
	private void SetSheet2(HSSFWorkbook wb, HSSFSheet sheet2, HSSFCellStyle style1, HSSFCellStyle style2,
		...
		try {
			...
			while (rs2.next()) {
				...
				cell.setCellType(1);
				cell.setCellStyle(style1C3);
				Double Intransit = getbuy(rs2.getString("EL_NO"), MODEL_NA, conn)
						- getacc(rs2.getString("EL_NO"), MODEL_NA, conn);
				...
				if (Integer.valueOf(His.get(1).toString()) > 0) {
					for (int k = 0; k < (Integer.valueOf(His.get(1).toString()) + 1); k++) {
						boolean is_buy = Check_buy(MODEL_NA, "DSID04_2", conn, k);
						if (!is_buy) {
							sheet2.setColumnHidden(12 + k, true);
						} else {
	...
	private void Header2(HSSFSheet sheet2, HSSFCellStyle style1, HSSFRow row, HSSFCell cell) {
		...
		for (int k = 0; k < (Integer.valueOf(His.get(1).toString()) + 1); k++) {
			sheet2.setColumnWidth(12 + k, 11 * 256);
		}
	...
	private void SetSheet1(HSSFWorkbook wb, HSSFSheet sheet1, HSSFCellStyle style1, HSSFCellStyle style2,
		...
		HSSFConditionalFormattingRule ruleRed = sheetCF.createConditionalFormattingRule(ComparisonOperator.EQUAL,
				"\"YES\"");
		HSSFFontFormatting fill = ruleRed.createFontFormatting(); // 字體色
		fill.setFontColorIndex(IndexedColors.RED.index);
		...
		try {
			...
			while (rs2.next()) {
				...
				cell.setCellValue(
						(rs2.getDate("ELDR_DATE").getTime() - (new Date()).getTime()) / (24 * 60 * 60 * 1000) + 1);
				...
				cell.setCellType(1);
				cell.setCellStyle(style2);
				cell.setCellFormula("IF($F$3<H" + (rowNum + 1) + ",\"Initial buy\",IF((J" + (rowNum + 1) + "-O"
						+ (rowNum + 1) + ")>AI" + (rowNum + 1) + ",\"Replenishment\",\"Ramp down\"))");
				...
				if (!"".equals(rs2.getString("EL_FP")) && rs2.getString("EL_FP") != null) {
					if (Double.valueOf(rs2.getString("EL_FP")) > 0) {
						propo = (MODEL_UPD * Double.valueOf(rs2.getString("YIELD"))
								* Double.valueOf(rs2.getString("COLOR_PRE"))) / Double.valueOf(rs2.getString("EL_FP"));
					} else {
						propo = 1.0;
					}
				} else {
				...
				if (!"".equals(rs2.getString("EL_NO")) && rs2.getString("EL_NO") != null) {
					...
					if (Integer.valueOf(His.get(1).toString()) > 0) {
						for (int k = 0; k < (Integer.valueOf(His.get(1).toString()) + 1); k++) {
							int HisQty = GetHisQty0(MODEL_NA, rs2.getString("EL_NO"), conn, k);
					...
					if (rs2.getString("COLOR").contains("/") && rs2.getString("COLOR").length() > 1) {
						...
						cell.setCellStyle(style32);
						cell.setCellValue(
								getkc2("DSID04_2", MODEL_NA, rs2.getString("COLOR").substring(0, 3), conn, conn));
				...
				cell.setCellType(1);
				cell.setCellStyle(style3);
				cell.setCellFormula("IF(ISERROR((S" + (rowNum + 1) + "/R" + (rowNum + 1) + ")/($AC$12*AC" + (rowNum + 1)
						+ ")),\"-\",(S" + (rowNum + 1) + "/R" + (rowNum + 1) + ")/($AC$12*AC" + (rowNum + 1) + "))");
				...
				cell.setCellType(1);
				cell.setCellStyle(style3);
				cell.setCellFormula("IF(ISERROR((Y" + (rowNum + 1) + "/R" + (rowNum + 1) + ")/($AC$12*AC" + (rowNum + 1)
						+ ")),\"-\",(Y" + (rowNum + 1) + "/R" + (rowNum + 1) + ")/($AC$12*AC" + (rowNum + 1) + "))");
				...
				cell.setCellFormula(
						"IF($F$3<H" + (rowNum + 1) + ",O" + (rowNum + 1) + "*$AB$3,O" + (rowNum + 1) + "*$AB$4)");

				cell = row.createCell(30);
				cell.setCellType(1);
				cell.setCellStyle(style3C1);
				cell.setCellFormula("IF($J" + (rowNum + 1) + "<(O" + (rowNum + 1) + "+AD" + (rowNum + 1) + "),$J"
						+ (rowNum + 1) + ",O" + (rowNum + 1) + "+AD" + (rowNum + 1) + ")");
				...
				cell.setCellType(1);
				cell.setCellStyle(style3C1);
				cell.setCellFormula("IF(AND($F$3<$H" + (rowNum + 1) + ",$K" + (rowNum + 1) + "<($O" + (rowNum + 1)
						+ "+$AD" + (rowNum + 1) + ")),$K" + (rowNum + 1) + "+$AD" + (rowNum + 1) + ",IF(J"
						+ (rowNum + 1) + "<O" + (rowNum + 1) + "+AD" + (rowNum + 1) + "+10,J" + (rowNum + 1) + ",O"
						+ (rowNum + 1) + "+AD" + (rowNum + 1) + "+20))");
				...
				cell.setCellType(1);
				cell.setCellStyle(style3C1);
				cell.setCellFormula("IF(AH" + (rowNum + 1) + "=\"YES\",IF(M" + (rowNum + 1) + "=\"initial buy\",(AI"
						+ (rowNum + 1) + "*AJ" + (rowNum + 1) + ")-Y" + (rowNum + 1) + ",IF(M" + (rowNum + 1)
						+ "=\"ramp down\",J" + (rowNum + 1) + "*AJ" + (rowNum + 1) + "-Y" + (rowNum + 1) + ",(AI"
						+ (rowNum + 1) + "+O" + (rowNum + 1) + ")*AJ" + (rowNum + 1) + "-Y" + (rowNum + 1) + ")),0)");
				...
				cell.setCellType(1);
				cell.setCellStyle(style3C2);
				cell.setCellFormula("IF(ISERROR(AO" + (rowNum + 1) + "/AJ" + (rowNum + 1) + "),\"-\",AO" + (rowNum + 1)
						+ "/AJ" + (rowNum + 1) + ")");
				...
				cell.setCellType(1);
				cell.setCellStyle(style3C2);
				cell.setCellFormula("IF(ISERROR((Y" + (rowNum + 1) + "+AO" + (rowNum + 1) + ")/AJ" + (rowNum + 1)
						+ "),\"-\",(Y" + (rowNum + 1) + "+AO" + (rowNum + 1) + ")/AJ" + (rowNum + 1) + ")");

				cell = row.createCell(44);
				cell.setCellType(1);
				cell.setCellStyle(style3);
				cell.setCellFormula("(Y" + (rowNum + 1) + "+AO" + (rowNum + 1) + ")-((I" + (rowNum + 1) + "-$F$3)*AJ"
						+ (rowNum + 1) + ")");
		...
		for (int k = 0; k < (Integer.valueOf(His.get(1).toString()) + 1); k++) {
			...
			if (k < 2) {
				...
				cell.setCellFormula("SUM(" + ((char) (66 + Integer.valueOf((k - 2) / 13)))
						+ ((char) (2 * (k - 2) + 65 - 26 * ((k - 2) / 13))) + "14:"
						+ ((char) (66 + Integer.valueOf((k - 2) / 13)))
						+ ((char) (2 * (k - 2) + 65 - 26 * ((k - 2) / 13))) + rowNum + ")");
			}
		...
		for (int k = 0; k < (Integer.valueOf(His.get(1).toString()) + 1); k++) {
			...
			for (int i = 13; i < rowNum; i++) {
				try {
					S_qty += sheet1.getRow(i).getCell(47 + 2 * k).getNumericCellValue();
				} catch (Exception e) {
					continue;
				}
			}
			if (S_qty == 0) {
				sheet1.setColumnHidden(47 + 2 * k, true);
				sheet1.setColumnHidden(48 + 2 * k, true);
	...
	private boolean Check_buy(String MODEL_NA, String table, Connection Conn, int k) {
		...
		if (k == 0) {
			...
			AddSql = " TO_DATE('" + His.get(2) + "','YYYY/MM/DD' )+" + (1 + (k - 1) * 7) + " AND TO_DATE('" + His.get(2)
					+ "','YYYY/MM/DD' )+" + (k * 7) + "";
		}

		String sql = "SELECT B.PO_QTY,B.PO_QTY-NVL((SELECT SUM(PC_QTY) FROM DSIDN08 WHERE PO_NO=B.PO_NO AND EL_NO=B.EL_NO),0) L_QTY FROM DSPO05 A,DSPO06 B WHERE A.PO_NO=B.PO_NO AND A.PO_NO LIKE 'IGM%'\n"
				+ "AND PO_QTY!=0 AND EL_NO IN (SELECT EL_NO FROM " + table
				+ " WHERE MODEL_NA=A.STOCK_MARK) AND STOCK_MARK='" + MODEL_NA + "'\n" + "AND A.PO_DATE BETWEEN"
				+ AddSql;
		try {
			ps = Conn.prepareStatement(sql, ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
			rs = ps.executeQuery();
	...
	private String GetHisQty(String MODEL_NA, String EL_NO, Connection Conn, int k) {
		...
		try {
			...
			rs = ps.executeQuery();
			while (rs.next()) {
				SQTY += Integer.valueOf(rs.getString("PO_QTY")); // 总数量
				L_QTY += Integer.valueOf(rs.getString("L_QTY")); // 未验收数量
			}
			rs.close();
			ps.close();
		} catch (Exception e) {
	...
	private void Header1(HSSFWorkbook wb, HSSFSheet sheet1, HSSFCellStyle style1, HSSFRow row, HSSFCell cell) {
		...
		List<String> heartname = Arrays.asList("Lgc Grp", "	Material Usage", "Fct Unique ID\n(FUI)", "Vendor",
				"Component Name", "Material Name \n(included size breaks if any)", "Color Code", "Launch date",
				"Drop date", "Days to drop from Todays date", "total lifespan days", "Starting date for initial buy",
				"Buying stages", "UOM", "MLT Days", "MOQ", "Unit Price", "Yield", "In Stock\n(UOM)", "In Stock\n($)",
				"In Stock\n(day)", "In Transit\n(UOM)", "In Transit\n($)", "ETA", "Current inventory\n(UOM)",
				"Current inventory\n(day)", "Current inventory\n(unit)", "Current inventory\n($)",
				"Order % by color or size", "Safety Days", "MIN Days", "MIN\n(UOM)", "MIN\n($)", "Order\n(Y/N)",
				"MAX Days", "Daily Demand\n(UOM)	", "MAX\n(UOM)", "Max\n($)", "Order Quantity\n(UOM)",
				" Order Quantity\n(USD)", "Factory Buy\n(UOM)", "Factory buy\n(days)", "Factory Buy\n(USD)",
		...
		try {
			...
			try {
				...
				while (rs.next()) {
					...
					String Upsql = "UPDATE DSID04_1 SET EL_FP='" + rs.getString("PROPO") + "' WHERE MODEL_NA='"
							+ MODEL_NA + "' AND EL_NO='" + rs.getString("EL_NO") + "'";
			...
			if (EL_NO_List.length() > 0) {
				...
				// 單個材料的 固定分配比例 為：1
				String Upsql2 = "UPDATE DSID04_1 SET EL_FP='' WHERE MODEL_NA='" + MODEL_NA + "' AND EL_NO NOT IN ('"
						+ EL_NO_List.replace(",", "','") + "')";
...
//				System.out.println("@@@@@我在這裡"+EL_PO);
	...
	private static Double getbuy2(String MODEL_NA, Connection conn) {
		...
		Double qty = 0.0;
		if (!"".equals(EL_NO_LIST) && EL_NO_LIST != null) {
	...
	private static Double getkc2(String TABLE, String MODEL_NA, String COLOR, Connection conn, Connection Conn) {
		...
		if (EL_NO_LIST.length() > 0) {
			...
			try {
				...
				rs = ps.executeQuery();
				if (rs.next()) {
					if (!"".equals(rs.getString("MT_QTY")) && rs.getString("MT_QTY") != null) {
						qty = Double.valueOf(rs.getString("MT_QTY"));
					}
				}
				rs.close();
