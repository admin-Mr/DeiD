...
public class ReadIDPicProgram extends QueryWindow {
	...
	@Wire private Textbox Barcode;
	@Wire private Datebox BarcodeDate;
	@Wire private Image imageMax, image0, image1, image2, image3;
	@Wire private Button Dayscanning, Anumberof, Sumowe, imageHome;
	@Wire private Button btnConfirm, btnRefresh, Networkimg, Serverimg;
	@Wire private Textbox Odno, OdDate, ShGroupID, WorkID, ModelM, LeftSize, SMdate;
	@Wire private Textbox WTNot_Neat, WTNot_Cancel, WTNot_Redo, WTNot_Repair, VIP_ID, Scnin_Date;
	@Wire private Textbox Cz1, Cz2, Cz3, Cz4, Zc1, Zc2, Zc3, Zc4, Cx1, Cx2, Cx3, Cx4, Zd1, Zd2, Zd3, Zd4, Ch1, Ch2, Ch3, Ch4;
	@Wire private Img img;
	...
	// 開始
	@Listen("onOK = #Barcode")
	public void onClickbtnConfirm() throws IOException {
		
		//String BcDate = "";
		String Bc = "", User = "";
		
		queue.add(Barcode.getValue());
		Bc = queue.poll();
		
		System.out.println(" 隊列Size :" + queue.size());
		
		if(Bc == "" || "".equals(Bc)){
			Messagebox.show(" 條碼欄不能為空 或 未检测到條碼扫描 !! ");
			return;
		}else{
			System.out.println(" ----- Bc.length() : " + Bc.length());
		...
		if (_userInfo.getAccount() == "XEID9" || "XEID9".equals(_userInfo.getAccount())) {
			...
			Bc = SetBarDate(BcDemo, BcDate);
			System.out.println(" ----- 組底Bc : " + Bc);
		}
		
		System.err.println(" ----- 開始 ----- Bc碼: " + Bc + " ----- Bc日期: " + BcDate + " ----- ID: " + Idname + " ----- 掃描時間: " + sDate);
		
		String[] Data = getData(Bc, BcDate, zcname); // 獲取訂單資料
	...
	public void onClickNetworkimg(String Bc) throws Exception {
		
			Img_SN = true;// 備註控制, 未使用
			
			String[] url = getUrl(Bc); // 默認為網絡圖片
	...
	public AImage[] onClickServerimg(String Bc) throws IOException {
		
		Img_SN = false;
		imageMax.setSrc("");
		System.err.println(" ----- Server Image" + Img_SN);
		
		String[] OdnoDate = getOdnoDate(Bc, BcDate); // 订单日期文件夹名, 用于拼凑根据日期选择文件夹
		
		System.err.println(" --- 测试 --- : " + OdnoDate[0] + " --- " +Bc+ "---" + BcDate );
		
		String[] data = getData(Bc, BcDate, zcname);  // 訂單資料獲取
		String[] Udata = getUrl(Bc);	// Url尾數獲取
		String[] Uwdata = Udata[4].split(",");
			...
			for(int i = 0; i < Uwdata.length; i++){
			    ...
			    String imgName = "", imgTail = "", Srcws = "", path = "";
			    String[] Srcs = null;
			    AImage aImg = null;
			    try {
			    	//Messagebox.show("1");
				    path = wb.getRealPath(File.separator+ "util" +File.separator+ "idimage" +File.separator+ OdnoDate[0]);
					//path = "//\\10.8.1.207\\erp_ftl_id\\ftl\\WEB-INF\\classes\\dsc\\echo2app\\resource\\idimage-1\\" + OdnoDate + "\\"; // 測試路徑
					
				    
					imgName = data[2];					// Ship_group_id
					imgTail = "-vw-";					// 尾戳
					Srcs = Uwdata[i].split("/");		// 尾數
					String str = Srcs[Srcs.length - 1];
					Srcws = str.substring(str.length() - 1); // 截取末尾數字
					//Messagebox.show(path + File.separator + imgName + imgTail + Srcws + ".jpg" + " ----- 1 -----"); // 路徑圖示
					...
					try {
						...
						//path = "//\\10.8.1.207\\erp_ftl_id\\ftl\\WEB-INF\\classes\\dsc\\echo2app\\resource\\idimage-1\\"; // 測試路徑
						...
						String str = Srcs[Srcs.length - 1];
						Srcws = str.substring(str.length() - 1); // 截取末尾數字
						//Messagebox.show(path + File.separator + imgName + imgTail + Srcws + ".jpg" + " ----- 2 -----"); // 路徑圖示
						aImg = new AImage(path + File.separator + imgName + imgTail + Srcws + ".jpg");
					} catch (Exception e2) {
						// TODO: handle exception
						...
						try {
							...
							//path = "//\\10.8.1.207\\erp_ftl_id\\ftl\\WEB-INF\\classes\\dsc\\echo2app\\resource\\idimage-1\\old\\"; // 測試路徑
							...
							String str = Srcs[Srcs.length - 1];
							Srcws = str.substring(str.length() - 1); // 截取末尾數字
							//Messagebox.show(path + File.separator + imgName + imgTail + Srcws + ".jpg" + " ----- 3 -----"); // 路徑圖示
							aImg = new AImage(path + File.separator + imgName + imgTail + Srcws + ".jpg");
						} catch (Exception e3) {
							// TODO: handle exception
						}
				...
				//System.out.println(" ----- Path: " + path + imgName + imgTail + Srcws + ".jpg");
	...
	public String[] getData(String Bc, String BcDate, String zcname) {
	...
	public String[] getState(String Bc, String user, String BcDate) throws IOException {
		...
		try {
			...
			if (rs.next()) {
				...
				//System.out.println(" ----- Data: " + state + " --- " + Isdel + " --- " + Isre + " --- " + IsRepair + " --- " + Postal + " --- " + Okdate);
	...
	// 獲取掃描圖片鏈接
	public String[] getUrl(String Bc) {
		...
		String sql = "select url1, url2, url3, url4 from dsid30_pic where work_order_id like '%" + Bc + "'";
		//System.out.println(" ----- Dsid30 Pic: " + sql);
		...
		try {
			...
			String sql1 = "select " + sqlurl1 + " url1, " + sqlurl2 + " url2, " + sqlurl3 + " url3, " + sqlurl4 + " url4 "
					+ "from dsid30 where work_order_id like '%" + Bc + "' ";
	...
	public void setReidDate(String Bc, String user) {
		...
		if(Bc != null && !"".equals(Bc) && Bc != "" ){
			String upDatesql = "update dsid65 set "+Zcname+" = to_date('" + date1 + "','yyyy/mm/dd') where work_order_id like '%"+ Bc + "'";
	...
	// 获取订单日期
	public String[] getOdnoDate(String Bc, String BcDate){
		...
		//if(_userInfo.getAccount() == "XEID9" || "XEID9".equals(_userInfo.getAccount())){
			//sql = "select work_order_id, order_date from dsid30 to_char(order_date,'YYYY/MM/DD') = '"+BcDate+"' and url1 = '"+Bcs+"'";
		//}else{
		...
		try {
			...
			Oredr[0] = OdnoDate;
			Oredr[1] = Work;
				//System.out.println(" ----- Date : " + rs.getString("order_date").substring(0,10).split("-"));
	...
	// 獲取組底製程Work_order_id
	private String SetBarDate(String BcDemo, String BcDate) {
...
/*	public static String getBc() {
	...
	public static void setBc(String bc) {
		Bc = bc;
	}*/
